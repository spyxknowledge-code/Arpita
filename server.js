...
db.serialize(() => {
  ...
  // Seed default users – but only if they don't exist (so sign-up can also create them)
  const defaultUsers = [
...
app.post('/api/signup', (req, res) => {
  ...
  if (!publicKey) {
    return res.status(400).json({ error: 'Public key required for encryption' });
  }
  // Check if username exists
  db.get('SELECT id FROM users WHERE username = ?', [username], (err, row) => {
    if (err) {
      console.error('DB error in signup check:', err);
      ...
      function(err) {
        ...
        db.run('INSERT INTO sessions (token, user_id, expires_at) VALUES (?, ?, ?)',
          ...
          (err) => {
            ...
            console.log(`✅ Sign-up success: ${username} (ID: ${userId})`);
...
// Get other user's info (for chat)
app.get('/api/other-user/:userId', async (req, res) => {
  ...
  // Find any other user (the one we chat with) – in this system, we pair user 1 with 2, but for general,
  // we find the first user who is not us and has a public key
  db.get('SELECT id, username, public_key, is_online FROM users WHERE id != ? AND public_key != "" ORDER BY id LIMIT 1', [myId], (err, row) => {
    if (err) {
      console.error('DB error in other-user:', err);
      return res.status(500).json({ error: 'Database error' });
    }
    if (!row) {
      return res.status(404).json({ error: 'No other user found. Ask a friend to sign up!' });
    }
...
io.on('connection', (socket) => {
  ...
  // Get the other user (any user who is not us)
  db.get('SELECT id FROM users WHERE id != ? AND public_key != "" ORDER BY id LIMIT 1', [userId], (err, row) => {
    if (row) {
      const otherId = row.id;
      socket.data.otherId = otherId;
  ...
  socket.on('mark_delivered', ({ messageId }) => {
    ...
    db.get('SELECT sender_id FROM messages WHERE id = ?', [messageId], (err, row) => {
      if (row) {
        io.to(`user_${row.sender_id}`).emit('delivered_receipt', { messageId });
      }
    });
  });
  ...
  socket.on('mark_read', ({ messageId }) => {
    ...
    db.get('SELECT sender_id FROM messages WHERE id = ?', [messageId], (err, row) => {
      if (row) {
        io.to(`user_${row.sender_id}`).emit('read_receipt', { messageId });
      }
    });
  });
