...
io.on('connection', (socket) => {
  ...
  db.get('SELECT username FROM users WHERE id = ?', [userId], (err, row) => {
    ...
    console.log(`✅ ${socket.data.username} (${userId}) connected | socket.id: ${socket.id}`);
  });

  // ----- JOIN ROOM -----
  socket.on('join-room', ({ roomId }) => {
    console.log(`📥 Join request: ${socket.data.username} (${socket.id}) -> room: ${roomId}`);
    db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
      ...
      db.get('SELECT user_id FROM room_members WHERE user_id = ? AND room_id = ?', [userId, roomRow.id], (err, member) => {
        ...
        console.log(`✅ ${socket.data.username} joined socket room: ${roomId} (socket.id: ${socket.id})`);
  ...
  socket.on('room-message', async ({ roomId, encryptedContent, iv, salt, replyToId }) => {
    console.log(`📨 Message from ${socket.data.username} (${socket.id}) in room ${roomId}`);
    ...
    db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
      ...
      db.run(
        ...
        function(err) {
          ...
          console.log(`📤 Message ${msgId} broadcast to room ${roomId} (total sockets in room: ${io.sockets.adapter.rooms.get(roomId)?.size || 0})`);
        }
      );
    });
  });
