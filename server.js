...
io.on('connection', (socket) => {
  ...
  db.get('SELECT username FROM users WHERE id = ?', [userId], (err, row) => {
    ...
    console.log(`✅ ${socket.data.username} connected, socket.id: ${socket.id}`);
  });

  socket.on('join-room', ({ roomId }) => {
    console.log(`📥 ${socket.data.username} joining room: ${roomId}`);
    db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
      ...
      db.get('SELECT user_id FROM room_members WHERE user_id = ? AND room_id = ?', [userId, roomRow.id], (err, member) => {
        ...
        console.log(`✅ ${socket.data.username} joined socket room: ${roomId}`);
  ...
  socket.on('room-message', async ({ roomId, encryptedContent, iv, salt, replyToId }) => {
    console.log(`📨 Message from ${socket.data.username} in ${roomId}`);
    ...
    db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
      ...
      db.run(
        ...
        function(err) {
          if (err) { console.error(err); socket.emit('message_error', 'Failed to save'); return; }
